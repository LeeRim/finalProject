  ...
  <div class="content-wrapper">
	...
	<c:forEach items="${list }" var="e2"  varStatus="status">
		<c:if test="${status.count eq 1 }">
			<li>
		<label>
    <input type="checkbox" value="${e2.eDepart }">
   <strong><b>${e2.eDepart }</b></strong>
  		</label>
			<c:forEach items="${list }" var="e3" >
			<c:if test="${e2.eDepart eq e3.eDepart }">
			<ul style="display: block;">
			<li>
				<label>
    <input type="checkbox" value="${e3.eName }">${e3.eName }
  				</label>
  				</li>
			</ul>
			</c:if>
   ...
   <strong><b>${e2.eDepart }</b></strong>
			...
			<c:forEach items="${list }" var="e4" >
			<c:if test="${e2.eDepart eq e4.eDepart }">
			<ul style="display: block;">
			<li>
				<label>
    <input type="checkbox" value="${e4.eName }">${e4.eName }
  				</label>
  				</li>
			</ul>
			</c:if>
		...
		<a href="NewFile.do"><button type="button" class="btn bg-olive margin">쪽지보내기</button></a>
		</div>
